...
#
RESOURCE_URL = '%(url)s%(resource)s' % {'url': os.environ["API_URL"], 'resource': util_resource.RESOURCE_URL_V1_TSE_PAIS}
...
select
    ...
    (
        ...
        (
            ...
            (
                'href', '%(_links_self)s/' || inner_query.pais_id
            ),
            'candidaturas', json_build_object
            (
                'href', '%(_links_self)s/' || inner_query.pais_id || '/%(_links_candidatura_sub_resource)s'
            ),
            'fontes', json_build_object
            (
                'href', '%(_links_self)s/' || inner_query.pais_id || '/%(_links_fonte_sub_resource)s'
            )
        )
    ) as json_object
--
