...
#
RESOURCE_URL = '%(url)s%(resource)s' % {'url': os.environ["API_URL"], 'resource': util_resource.RESOURCE_URL_V1_TSE_MUNICIPIO}
...
select
    ...
    (
		...
		(
			...
			(
                ...
                (
					...
					(
						'href', '%(_embedded_unidade_federativa_links_self)s/' || inner_query.municipio_id_unidade_federativa
					)
				)
			)
		),
        ...
        (
            'self', json_build_object
            (
                'href', '%(_links_self)s/' || inner_query.municipio_id
            ),
            'candidaturas', json_build_object
            (
                'href', '%(_links_self)s/' || inner_query.municipio_id || '/%(_links_candidatura_sub_resource)s'
            ),            
            'fontes', json_build_object
            (
                'href', '%(_links_self)s/' || inner_query.municipio_id || '/%(_links_fonte_sub_resource)s'
            )
        )
    ) as json_object
--
...
(
    ...
    select distinct
        ...
        municipio.id_unidade_federativa as municipio_id_unidade_federativa,
...
#
class TseMunicipioResourceService:
