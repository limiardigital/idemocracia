...
#
v1_tse_candidatura_bem_routes = Blueprint(__name__)

#
service = tse_candidatura_bem_resource_service.TseCandidaturaBemResourceService()
fonte_service = tse_fonte_resource_service.TseFonteResourceService()

#
@v1_tse_candidatura_bem_routes.route('%(resource)s/{id}' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA_BEM}, methods=['GET'], cors=True)
def get_by_id(id):
...
#
@v1_tse_candidatura_bem_routes.route('%(resource)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA_BEM}, methods=['GET'], cors=True)
def get_all():
...
#
@v1_tse_candidatura_bem_routes.route('%(resource)s/{id}/%(sub_resource_fonte)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA_BEM, 'sub_resource_fonte': util_resource.RESOURCE_V1_TSE_FONTE}, methods=['GET'], cors=True)
def get_all_fonte(id):
    ...
    try:
        ...
        result = fonte_service.find_all_by(
            ...
            page_number=query_param_page_number,
            page_size=query_param_page_size,
            id_candidatura_bem=path_param_id)
