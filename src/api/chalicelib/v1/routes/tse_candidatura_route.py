...
from chalicelib.v1.services import tse_candidatura_bem_resource_service
from chalicelib.v1.services import tse_fonte_resource_service

#
v1_tse_candidatura_routes = Blueprint(__name__)

#
service = tse_candidatura_resource_service.TseCandidaturaResourceService()
candidatura_bem_service = tse_candidatura_bem_resource_service.TseCandidaturaBemResourceService()
fonte_service = tse_fonte_resource_service.TseFonteResourceService()

#
@v1_tse_candidatura_routes.route('%(resource)s/{id}' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA}, methods=['GET'], cors=True)
def get_by_id(id):
...
#
@v1_tse_candidatura_routes.route('%(resource)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA}, methods=['GET'], cors=True)
def get_all():
...
#
@v1_tse_candidatura_routes.route('%(resource)s/{id}/%(sub_resource_fonte)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA, 'sub_resource_fonte': util_resource.RESOURCE_V1_TSE_CANDIDATURA_BEM}, methods=['GET'], cors=True)
def get_all_candidatura_bem(id):
    ...
    try:
        ...
        print(app.app.current_request.context)

        #
        result = candidatura_bem_service.find_all_by(
            origin_resource_path=app.app.current_request.context['path'],
            page_number=query_param_page_number,
            page_size=query_param_page_size,
            id_candidatura=path_param_id)
...
#
@v1_tse_candidatura_routes.route('%(resource)s/{id}/%(sub_resource_fonte)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_CANDIDATURA, 'sub_resource_fonte': util_resource.RESOURCE_V1_TSE_FONTE}, methods=['GET'], cors=True)
