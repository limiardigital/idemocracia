...
#
v1_tse_unidade_federativa_routes = Blueprint(__name__)

#
service = tse_unidade_federativa_resource_service.TseUnidadeFederativaResourceService()
...
#
@v1_tse_unidade_federativa_routes.route('%(resource)s/{id}' % {'resource': util_resource.RESOURCE_URL_V1_TSE_UNIDADE_FEDERATIVA}, methods=['GET'], cors=True)
def get_by_id(id):
...
#
@v1_tse_unidade_federativa_routes.route('%(resource)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_UNIDADE_FEDERATIVA}, methods=['GET'], cors=True)
def get_all():
...
#
@v1_tse_unidade_federativa_routes.route('%(resource)s/{id}/%(sub_resource_fonte)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_UNIDADE_FEDERATIVA, 'sub_resource_fonte': util_resource.RESOURCE_V1_TSE_CANDIDATURA}, methods=['GET'], cors=True)
def get_all_candidatura(id):
    ...
    try:
        ...
        result = candidatura_service.find_all_by(
            ...
            page_number=query_param_page_number,
            page_size=query_param_page_size,
            id_unidade_federativa=path_param_id)
...
#
@v1_tse_unidade_federativa_routes.route('%(resource)s/{id}/%(sub_resource_fonte)s' % {'resource': util_resource.RESOURCE_URL_V1_TSE_UNIDADE_FEDERATIVA, 'sub_resource_fonte': util_resource.RESOURCE_V1_TSE_FONTE}, methods=['GET'], cors=True)
